...
interface PackagingPackerPageProps {
	user?: UserType;
	// users?: UserType[];
}
...
const PackagingPackerPage: React.FC<PackagingPackerPageProps> = (props) => {
	...
	async function handleClick() {
		const result = httpRequest<UserType[]>('get', 'http://ithoma.net/admins/getMulti',
			{
				params: {
            ...
            <DefaultLayout>
				<section className="flex flex-col items-center justify-center gap-1 py-0 md:py-4">
					<div className="flex flex-col px-4 py-4 items-center text-center justify-center bg-onBackground">
						<Marketer className="m-4"/>
						<h1 className={title()}> مسئول بسته بندی </h1>
						{/* <h1 className={subtitle()}>لیست بسته بند ها</h1>
...
export default PackagingPackerPage;
