...
const PurchaseSupplierPage: React.FC<PurchaseManagerPageProps> = (props) => {
			...
			result.then(value => 
				{
					if (value.data ) {
						...
						for (const doc of value.data) {
							// list.push(doc.purchases[doc.purchases.length-1])
	...
	function handleBuyClick() {
		...
		// console.log(JSON.parse(basketList))
			...
			<DefaultLayout>
				<section className="flex flex-col items-center justify-center py-0 md:py-4">
					<div className="flex flex-col px-4 py-4 items-center text-center justify-center bg-onBackground">
						<Marketer />
						<h1 className={title()}>مسول خرید</h1>
	...
	//     return (
	//         <DefaultLayout>
	//             <div className='flex justify-center pl-8 pr-8 bg-onbackground'>
	// 				<div className={colClass} style={{height: 500}}>
	// 					<p className={textClass}>Products :</p>
	// 					<div style={{ overflowY: 'auto', whiteSpace: 'nowrap' }} className='colClass'>	
	// 						<ProductTable onClick={handleClick} products={JSON.parse(products)}/>
	// 					</div>
