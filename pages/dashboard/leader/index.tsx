...
const LeaderPage: React.FC<LeaderPageProps> = (props: any) => {
	...
	function handleClick(user: UserType) {
		if (user) {
			const result = httpRequest<UserType[]>('get', 'http://ithoma.net:8080/admins/getMulti',
					{
						params: {
							id: user.mobile,
						},
			...
			<DefaultLayout>
				<section className="flex flex-col items-center justify-center gap-1 py-0 md:py-4">
					<div className="flex flex-col px-4 py-4 items-center text-center justify-center bg-onBackground">
						<Leader />
						<h1 className={title()}>مدیر</h1>
						<h1 className={subtitle()}>لیست همه کاربران</h1>
